...
Bring Vulkan 1.2 rendering to Minecraft Java for smoother visuals, lower CPU overhead, and access to modern GPU features.
...
## Overview
VulkanMod is an experimental Fabric client mod that replaces Minecraft Java's OpenGL 3.2 renderer with a custom Vulkan 1.2 pipeline. The goal is to modernize the engine, reduce CPU overhead, and unlock rendering features that are difficult or impossible with OpenGL.
...
For support, read the [Wiki](https://github.com/xCollateral/VulkanMod/wiki) first, then visit the [Discord server](https://discord.gg/FVXg7AYR2Q) or [open an issue](https://github.com/xCollateral/VulkanMod/issues) with full logs.
...
## Project Notes
- VulkanMod is a full renderer rewrite, not a translation layer like [Mesa's Zink](https://docs.mesa3d.org/drivers/zink.html).
...
- Redirect verbose commands to a log (for example `./gradlew genSource > genSource.log`) if you need to inspect output later.
